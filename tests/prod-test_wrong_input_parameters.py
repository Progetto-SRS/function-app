...
response = requests.get("https://prod-functions-srs.azurewebsites.net/api/create-container")
assert response.status_code == 400
...
response = requests.get("https://prod-functions-srs.azurewebsites.net/api/delete-container")
assert response.status_code == 400
assert response.content == b"ERRORE -> Mancano i parametri nomeContainer e accountStorage"
